...
class OnlineMenuState extends FlxState
{
    ...
    override public function create()
    {
        ...
        joinLobby = new FlxSprite().loadGraphic(AssetPaths.joinlobby__png);
        joinLobby.x = 176.5;
        joinLobby.screenCenter(Y);
        add(joinLobby);

        makeLobby = new FlxSprite().loadGraphic(AssetPaths.makelobby__png);
        makeLobby.x = 776.5;
        ...
        if (!selectedsum)
        {
            ...
            if (FlxG.keys.justPressed.ENTER)
            {
                ...
                switch(curSelected)
                {
                    ...
                    case 1: //create
                        SessionData.start(NetworkMode.SERVER, {ip: '127.0.0.1', port: 9696, max_connections: 4, uuid: 'poop'});
