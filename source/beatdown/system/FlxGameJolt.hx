...
class FlxGameJolt
{
	...
	public static var _otherThingy:Dynamic; // this is because the callback won't stay the same for the fucking life of me so

	static var _imgSize:Float;
	...
	static var loader:URLLoader; // other one
	...
	public static function fetchUser(?UserID:Int, ?UserName:String, ?UserIDs:Array<Int>, ?Callback:Dynamic):Void
	{
		...
		if (_otherThingy == null)
			_otherThingy = Callback;
	...
	static function sendLoaderRequest(URLString:String, ?Callback:Dynamic):Void
	{
		...
		if (verbose)
			trace("FlxGameJolt: Contacting " + request.url);
	...
	static function parseData(e:Event):Void
	{
		...
		{
			trace("FlxGameJolt received no data back. This is probably because one of the values it was passed is wrong.");
			return;
		}
		...
		if (returnMap.exists("message") && verbose)
			trace("FlxGameJolt: GameJolt returned the following message: " + returnMap.get("message"));
	 ...
	 * @param	Callback	An optional callback function. Must take a BitmapData object as a parameter.
	 */
	public static function fetchAvatarImage(size:Float = 300, ?Callback:BitmapData->Void):Void
	{
		if (!gameInit)
			return;
		_imgSize = size;
	...
	static function retrieveImage(ImageMap:Map<String, String>):Void
	{
		if (ImageMap.exists("image_url"))
		{
			...
			loader.addEventListener(Event.COMPLETE, returnImage);
			loader.load(new URLRequest(ImageMap.get("image_url")));
		}
		else if (ImageMap.exists("avatar_url"))
		{
			var thing:String = ImageMap.get("avatar_url");
			thing = thing.replace('.webp', '.jpg');
			thing = thing.replace('/60/', '/${_imgSize}/');

			loader = new URLLoader();
			loader.dataFormat = URLLoaderDataFormat.BINARY;
			loader.addEventListener(Event.COMPLETE, returnImage);
			loader.load(new URLRequest(thing));
		}
		else
		{
			trace("FlxGameJolt: Failed to load image");
		}
	}
	...
	{
		trace(_callBack != null);
		_callBack = _otherThingy;

		if (_callBack != null)
			_callBack(BitmapData.fromBytes(loader.data));
	...
	static function get_gameInit():Bool
	{
		if (_gameID == 0 || _privateKey == "")
		{
			trace("FlxGameJolt: You must run init() before you can do this. Game ID is "
				+ _gameID
				+ " and the key is "
				+ _privateKey
				+ ".");
	...
	static function get_authenticated():Bool
	{
		...
		if (!_initialized)
		{
			trace("FlxGameJolt: You must authenticate user before you can do this.");
			return false;
		}
